...
class MultiDomainSIDLanguageModel(SIDLanguageModel):
    ...
    def losses(self, records, with_proxy=False):
        if with_proxy:
            raise ValueError("Use exact legal-loss gradients; readout proxy is not defined here")
        ...
        logits = core.get_output_embeddings()(hidden[row, col])
        allowed, labels = [], []
        for record in records:
            choices, targets = self.legal_layout[(record.task, record.answer)]
            allowed.extend(choices)
            labels.extend(targets)
        losses = legal_cross_entropy(logits, allowed, labels).reshape(len(records), 4).mean(1)
